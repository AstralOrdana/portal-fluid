 ...
 * <p>In order to mark an item as capable of changing this text, you must implement {@link RhymingGaslightTooltipItem} on the class, and add {@link RhymingGaslightTooltipState#getText()} to the list of text components via {@link net.minecraft.world.item.Item#appendHoverText(ItemStack, Item.TooltipContext, List, TooltipFlag)}. Two of these items next to each other in an inventory will not re-shuffle the tooltip text when mouse-hopped between; the cursor must first move to a slot with a different (or empty) item before changing the text on next viewing.</p>
 *
...
public enum RhymingGaslightTooltipState {
    ...
    FEARS("tooltip.dimensional_tears.rhymes_with_tears_0"),
    SCARES("tooltip.dimensional_tears.rhymes_with_tears_1");
