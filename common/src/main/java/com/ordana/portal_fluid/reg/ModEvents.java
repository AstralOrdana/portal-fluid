...
import com.ordana.portal_fluid.util.InteractionEvent;
...
import static com.ordana.portal_fluid.blocks.PortalFluidCauldronBlock.tryCollectWithBottle;
import static com.ordana.portal_fluid.items.PortalFluidBottleItem.tryCollectObsidianTears;
import static com.ordana.portal_fluid.items.PortalFluidBottleItem.tryCollectRespawnAnchorTears;
...
public class ModEvents {
    ...
    static {
        EVENTS.add(ModEvents::glassBottle);
        EVENTS.add(ModEvents::bucket);
    }
    ...
    public static InteractionResult onBlockClicked(ItemStack itemStack, Player player, Level level, InteractionHand interactionHand, BlockHitResult blockHitResult) {
        ...
        for (InteractionEvent event : EVENTS) {
            InteractionResult result = event.run(itemStack, pos, state, player, level, interactionHand, blockHitResult);
    ...
    private static InteractionResult glassBottle(ItemStack itemStack, BlockPos blockPos, BlockState blockState, Player player, Level level, InteractionHand interactionHand, BlockHitResult blockHitResult) {
        ...
        return switch (blockState.getBlock()) {
            case PortalFluidCauldronBlock ignored ->
                tryCollectWithBottle(itemStack, blockPos, blockState, player, level, interactionHand);
            case CryingObsidianBlock ignored ->
                tryCollectObsidianTears(level, blockPos, itemStack, player, interactionHand);
            case RespawnAnchorBlock ignored ->
                tryCollectRespawnAnchorTears(level, blockPos, blockState, itemStack, player, interactionHand);
