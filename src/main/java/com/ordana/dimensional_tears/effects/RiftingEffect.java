...
public class RiftingEffect extends CountdownEffect {
    ...
    public static int teleportDelayTicks() {
        return DimensionalTearsRoot.CONFIG.teleportation.RIFTING_DELAY_SECONDS * SharedConstants.TICKS_PER_SECOND;
    }
    ...
    public static void tryAddInstanceParticles(ServerLevel serverLevel, LivingEntity livingEntity, MobEffectInstance mobEffectInstance) {
        ...
        if (duration > 1) {
            ...
            RiftingParticleS2CMessage.send(serverLevel, livingEntity, (byte) Mth.lerpDiscrete(f * f * f, 0, DimensionalTearsClient.CONFIG.particles.EFFECT_PARTICLE_DENSITY), f, false);
    ...
    public static boolean has(LivingEntity livingEntity) {
        return livingEntity.hasEffect(ModEffects.RIFTING);
    }
